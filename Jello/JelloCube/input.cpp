...
void mouseButton(int button, int state, int x, int y)
{
  switch (button)
  {
    case GLUT_LEFT_BUTTON:
      ...
      {
          isMouseForce=true;
          DownPos[0]=g_vMousePos[0];DownPos[1]=g_vMousePos[1];
          UpPos[0]=x;UpPos[1]=y;
          printf("Start from x:%d y:%d to x:%d y:%d\n",DownPos[0],UpPos[1],UpPos[0],DownPos[1]);
      }
      break;
...
/* writes the  parameters to a world file on disk*/
